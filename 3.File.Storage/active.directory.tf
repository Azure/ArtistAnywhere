...
resource azurerm_resource_group active_directory {
  ...
  name     = "${module.global.resourceGroupName}.AD"
...
resource azurerm_network_interface active_directory {
 ...
 resource azurerm_windows_virtual_machine active_directory {
  ...
  resource_group_name = azurerm_resource_group.active_directory[0].name
  location            = azurerm_resource_group.active_directory[0].location
  source_image_id     = "/subscriptions/${module.global.subscriptionId}/resourceGroups/${var.activeDirectory.machine.image.resourceGroupName}/providers/Microsoft.Compute/galleries/${var.activeDirectory.machine.image.galleryName}/images/${var.activeDirectory.machine.image.definitionName}/versions/${var.activeDirectory.machine.image.versionId}"
...
resource azurerm_virtual_machine_extension active_directory {
  ...
  depends_on = [
    azurerm_windows_virtual_machine.active_directory
  ]
}
