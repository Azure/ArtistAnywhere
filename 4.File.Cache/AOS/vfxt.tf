...
locals {
  vfxtCache = merge(var.vfxtCache, {
    cluster = merge(var.vfxtCache.cluster, {
      ...
      sshKeyPublic  = var.vfxtCache.cluster.sshKeyPublic != "" ? var.vfxtCache.cluster.sshKeyPublic : "${data.azurerm_key_vault_secret.ssh_key_public.value} admin@vfxt"
    })
  })
...
module vfxt_controller {
  ...
  virtual_network_resource_group = data.azurerm_virtual_network.main.resource_group_name
...
resource avere_vfxt cache {
  ...
  azure_network_name              = data.azurerm_virtual_network.main.name
  azure_network_resource_group    = data.azurerm_virtual_network.main.resource_group_name
