...
resource azapi_resource fleet {
  ...
  parent_id = azurerm_resource_group.cluster_fleet[0].id
  location  = azurerm_resource_group.cluster_fleet[0].location
  identity {
    type = "UserAssigned"
    identity_ids = [
            ...
            extensions = [
              { # Custom
                ...
                properties = {
                  ...
                  typeHandlerVersion      = lower(each.value.machine.osDisk.type) == "windows" ? data.azurerm_app_configuration_keys.main.items[index(data.azurerm_app_configuration_keys.main.items[*].key, data.terraform_remote_state.foundation.outputs.appConfig.key.scriptExtensionWindows)].value : data.azurerm_app_configuration_keys.main.items[index(data.azurerm_app_configuration_keys.main.items[*].key, data.terraform_remote_state.foundation.outputs.appConfig.key.scriptExtensionLinux)].value
